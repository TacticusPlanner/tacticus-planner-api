...
- Raw sources live under `Data/**`, discovered by convention (dataset key `foo-bar` → embedded `foo-bar.json`, matched by leaf filename; subfolders don't matter). `Models/GameCatalogDatasets.cs` is the registry of raw source keys and the `Served` list. Large entities are authored as a **family of raw files** enumerated in a static key list and merged at load — `units-<faction>` (22), `npcs-<faction>` (21), `lres-<event>` + `lre-common`, `shops-<source>` (4), `campaign-battles-<group>` (22).
...
**8. Raw source is authored as a per-boss + per-season + shared-common file family, merged at load; the served dataset is unchanged.**
`Data/raid-bosses/` holds `raid-boss-<n>-<Type>.json` (one file per raid boss `n` — its boss/prime/field-npc/loot-object unit sets; `<Type>` is the boss's `Boss1` unit-set key with the `GuildBoss<n>Boss1` prefix stripped, e.g. `raid-boss-11-TauRiptide.json`, kept verbatim so the filename traces straight back to the key), `raid-boss-season-<n>.json` (one per guild-raid season config), and `raid-boss-common.json` (season-config rotation, primarch prime ids, modifier definitions). `GameCatalogDatasets` enumerates them as `RaidBossGroups`, `RaidBossSeasons`, and the `RaidBossCommon` key. `GameCatalogLoader.LoadRaidBossRawData` merges the boss files' `UnitSets` and keys the season files by their own `GuildBossSeasonConfigId` into the same `GameCatalogRaidBossRawData` the denormalizer already consumes — so `BuildRaidBosses`, validation, the served `raid-bosses` projection, and its manifest hash are all untouched. A duplicate unit-set key or season-config id across files throws at load.
- _Why not one `raid-boss-data.json`_: the consolidated file was ~1.3 MB pretty-printed — 4x the next-largest catalog source and an outlier against the file-family convention every other large entity follows. A re-port after a game balance patch produced one unreviewable mega-diff; the split localises it to the boss/season file(s) that actually changed. This mirrors `lres-<event>` + `lre-common` exactly.
- _Why per-season files, not one `raid-boss-seasons.json`_: a season config is ~54 KB of tier/set/encounter tree and the five are edited independently across game updates; one file per season keeps each diff isolated, same rationale as the per-boss split.
- _Boundary_: unit sets partition cleanly by the `GuildBoss<n>` key prefix (every key carries it). Season configs do **not** belong to a single boss — their encounters reference units across many bosses — so they are their own file family, and the truly global bits (rotation, primarchs, modifier defs) land in `raid-boss-common.json`.

## Risks / Trade-offs

- [Trade-off] The consolidated *served* dataset embeds every boss's full progression ladder and every season's encounter tree in one payload. Estimated well under the raw 1.9 MB after dropping `misc`/`boards` and display-only fields, but larger than most single datasets. Accepted — it's downloaded once and cached in IndexedDB, and splitting it (Decision 1 alternative) trades size for a client-side join. Revisit if the manifest diff shows it dominating sync cost. (The *raw* source is split for authoring ergonomics — Decision 8 — independent of the served shape.)
