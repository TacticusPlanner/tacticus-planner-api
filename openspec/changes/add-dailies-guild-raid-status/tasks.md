...
- [ ] 1.3 Implement boss/prime HP and proportional modifier-threshold projection and verify focused tests cover catalog HP fallback, active/inactive modifiers, and exact null `activationRemainingHp`/`active` values when prime HP is unresolved.
...
- [ ] 2.1 Add the guild-scoped Guild Raid sync state plus season, hit, and hit-unit persistence model and EF Core migration, with guild/season ownership, cascade behavior, deterministic unique/idempotent hit identity, and a `(GuildRaidSeasonId, TacticusUserIdHash, CompletedAt)` composite index; verify PostgreSQL integration tests cover active/no-active state, constraints, repeated writes, and indexed current-user ordering.
- [ ] 2.2 Implement transactional upstream normalization and the per-guild five-minute fresh persisted-observation/single-flight refresh service; verify concurrency/time-controlled tests cover fresh reuse, forced refresh, coalescing, overlapping idempotent writes, transient persisted stale fallback, rejected-credential behavior with retained data, and process-restart reuse.
- [ ] 2.3 Add a no-tracking current-user hit repository projection that accepts season id plus the caller's existing Tacticus user-id hash, selects only required columns, and restricts optional unit loading to matching hit ids; verify relational query tests prove another member's hits/units are not materialized and the SQL predicates match the composite index.
- [ ] 2.4 Add the authenticated `GET /api/v1/guilds/me/raid-status` endpoint with guild-readiness/token lookup and upstream error mapping, and verify endpoint tests cover unprovisioned, unregistered, never-synced, active, no-season, rejected, unavailable, and persisted stale-fallback responses without leaking credentials or raw hits.
- [ ] 2.5 Register the persistence/status services and verify the API test host resolves the endpoint and one request uses the fake upstream client plus the current catalog snapshot.
