...
- [x] 1.1 Add `string? QuestUnitId` as the last optional field on
...
- [x] 1.2 Map `rawUnitSet.QuestUnitId` onto the view in `RaidBossDenormalizer`;
...
- [x] 2.1 Extend the `GameCatalog.Tests` raid-boss denormalization test: assert
...
- [x] 2.2 Regenerate the catalog manifest snapshot
...
- [x] 3.1 Run `dotnet format TacticusPlanner.slnx --verify-no-changes --no-restore`,
  `dotnet build TacticusPlanner.slnx -c Release --no-restore`, and
  `dotnet test TacticusPlanner.slnx -c Release --no-build`; all green.
- [x] 3.2 Confirm the regenerated `artifacts/openapi` diff is empty (no endpoint
